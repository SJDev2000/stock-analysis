...
description: Fetch and score Stocktwits stream for a ticker. Returns a compact structured JSON block with per-message scores, aggregate stats, and labeled vs unlabeled breakdown. Designed to be invoked as a context fork from the parent sentiment-analysis skill.
context: fork
tools:
...
1. Call `fetch_stocktwits_stream` with the ticker to get the asset path.
2. Run `!cat {asset_path}` to load all messages.
3. Score every message using the rules below.
